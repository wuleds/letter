 ...
 */
@Service
public class RequestServiceImpl implements RequestLogService {
    @Resource
    RequestLogDao requestLogDao;

    @Override
    public void insertLog(RequestLog requestLog) {
        requestLogDao.insertLog(UUIDUtil.getUUID(),requestLog.getIp(),
                requestLog.getHost(),requestLog.getUri(),requestLog.getCode(),
                requestLog.getMsg(),requestLog.getUserName(),requestLog.getUserId(),requestLog.getUserInfo());
    }
}