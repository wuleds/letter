...
{
    /**为新注册的用户添加联系人表列*/
    @Insert("insert into contact (user_id,contact_list) values (#{userId},'')")
   ...
   /**获取所有的添加联系人请求*/
    ...
    /**获取黑名单列表*/
    @Select("select black_list from black_list where id = #{userId} and del_flag= 0")
    String getBlackList(String userId);

    /**更新黑名单列表*/
    @Update("update black_list set black_list = #{blackList} where id = #{userId} and del_flag = 0")
